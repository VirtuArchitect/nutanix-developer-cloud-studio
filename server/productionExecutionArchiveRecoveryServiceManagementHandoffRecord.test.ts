...
import type { ProductionExecutionArchiveRecoveryOperationalContinuityRecord } from "../src/data/cloudStudioDomain";
import {
...
describe("production execution archive recovery service management handoff record", () => {
  it("requires a production execution archive recovery operational continuity record", () => {
    const state = createDefaultState();

    expect(() =>
      createProductionExecutionArchiveRecoveryServiceManagementHandoffRecord(state, {}, "platform.admin")
    ).toThrow(ProductionExecutionArchiveRecoveryServiceManagementHandoffRecordError);
  });

  it("blocks service management handoff when continuity or service evidence is incomplete", () => {
    const state = createDefaultState();
    state.productionExecutionArchiveRecoveryOperationalContinuityRecords = [
      sampleOperationalContinuityRecord("Blocked"),
    ];
  ...
  it("marks service management handoff ready when continuity and service evidence are complete", () => {
    const state = createDefaultState();
    state.productionExecutionArchiveRecoveryOperationalContinuityRecords = [
      sampleOperationalContinuityRecord(
    ...
    expect(record.serviceOwner).toBe("Production Archive Recovery Service Owner");
    expect(record.supportQueueMappingReference).toBe(
      "production-archive-recovery-service-management-support-queue-ndb.json"
    );
    expect(record.knowledgeArticleReference).toBe(
      "production-archive-recovery-service-management-knowledge-article-ndb.md"
    );
    expect(record.escalationMatrixReference).toBe(
      "production-archive-recovery-service-management-escalation-matrix-ndb.md"
    );
    expect(record.serviceManagementHandoffSignOffReference).toBe(
...
function sampleOperationalContinuityRecord(
  ...
  return {
    id: "production-execution-archive-recovery-operational-continuity-record-ndb-1",
    provider: "NDB",
    evidenceCustodyClosureRecordId:
      "production-execution-archive-recovery-evidence-custody-closure-record-ndb-1",
    ...
    continuityOwner: "Production Archive Recovery Continuity Owner",
    runbookUpdateReference: "production-archive-recovery-operational-continuity-runbook-ndb.md",
    kpiBaselineReference: "production-archive-recovery-operational-continuity-kpi-baseline-ndb.json",
    supportHandoffReference: "production-archive-recovery-operational-continuity-support-handoff-ndb.md",
    continuitySignOffReference: "production-archive-recovery-operational-continuity-signoff-ndb.md",
    checks: [],
    evidence: [],
