...
import type { ProductionExecutionArchiveRecoveryAuditCertificationRecord } from "../src/data/cloudStudioDomain";
import {
...
describe("production execution archive recovery final compliance archive record", () => {
  it("requires a production execution archive recovery audit certification record", () => {
    const state = createDefaultState();

    expect(() =>
      createProductionExecutionArchiveRecoveryFinalComplianceArchiveRecord(state, {}, "platform.admin")
    ).toThrow(ProductionExecutionArchiveRecoveryFinalComplianceArchiveRecordError);
  });

  it("blocks final compliance archive when certification or archive evidence is incomplete", () => {
    const state = createDefaultState();
    state.productionExecutionArchiveRecoveryAuditCertificationRecords = [sampleAuditCertificationRecord("Blocked")];
    ...
    expect(record.checks).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ name: "Compliance archive owner assigned", passed: false }),
        expect.objectContaining({ name: "Prototype does not execute adapter", passed: true }),
      ])
    );
  });

  it("marks final compliance archive ready when certification and archive evidence are complete", () => {
    const state = createDefaultState();
    state.productionExecutionArchiveRecoveryAuditCertificationRecords = [
      sampleAuditCertificationRecord("Ready for production execution archive recovery audit certification review"),
    ];
    ...
    expect(record.complianceArchiveOwner).toBe("Production Archive Recovery Final Compliance Archive Owner");
    expect(record.finalComplianceArchiveIndexReference).toBe(
      "production-archive-recovery-final-compliance-archive-index-ndb.json"
    );
    expect(record.evidenceRetentionProofReference).toBe(
      "production-archive-recovery-final-compliance-retention-proof-ndb.md"
    );
    expect(record.auditWitnessReceiptReference).toBe("production-archive-recovery-audit-witness-receipt-ndb.md");
    expect(record.finalComplianceArchiveSignOffReference).toBe(
...
function sampleAuditCertificationRecord(
  ...
  return {
    id: "production-execution-archive-recovery-audit-certification-record-ndb-1",
    provider: "NDB",
    archiveRecoveryClosureRecordId: "production-execution-archive-recovery-closure-record-ndb-1",
    ...
    certificationOwner: "Production Archive Recovery Audit Certification Owner",
    auditEvidenceManifestReference: "production-archive-recovery-audit-evidence-manifest-ndb.json",
    controlMappingReviewReference: "production-archive-recovery-control-mapping-review-ndb.md",
    exceptionDispositionReference: "production-archive-recovery-exception-disposition-ndb.md",
    auditCertificationSignOffReference: "production-archive-recovery-audit-certification-signoff-ndb.md",
    checks: [],
    evidence: [],
