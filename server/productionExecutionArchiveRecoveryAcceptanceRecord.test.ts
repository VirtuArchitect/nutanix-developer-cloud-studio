...
import type { ProductionExecutionArchiveRecoveryDrillRecord } from "../src/data/cloudStudioDomain";
import {
...
describe("production execution archive recovery acceptance record", () => {
  it("requires a production execution archive recovery drill record", () => {
    const state = createDefaultState();

    expect(() =>
      createProductionExecutionArchiveRecoveryAcceptanceRecord(state, {}, "platform.admin")
    ).toThrow(ProductionExecutionArchiveRecoveryAcceptanceRecordError);
  });

  it("blocks archive recovery acceptance when drill or acceptance evidence is incomplete", () => {
    const state = createDefaultState();
    state.productionExecutionArchiveRecoveryDrillRecords = [sampleArchiveRecoveryDrillRecord("Blocked")];

    const record = createProductionExecutionArchiveRecoveryAcceptanceRecord(
      state,
      {
    ...
    expect(record.checks).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ name: "Acceptance owner assigned", passed: false }),
        expect.objectContaining({ name: "Prototype does not execute adapter", passed: true }),
      ])
    );
  });

  it("marks archive recovery acceptance ready when drill and acceptance evidence are complete", () => {
    const state = createDefaultState();
    state.productionExecutionArchiveRecoveryDrillRecords = [
      sampleArchiveRecoveryDrillRecord("Ready for production execution archive recovery drill review"),
    ];
    ...
    expect(record.status).toBe("Ready for production execution archive recovery acceptance review");
    expect(record.acceptanceOwner).toBe("Production Archive Recovery Acceptance Owner");
    expect(record.recoveryEvidencePacketReference).toBe("production-archive-recovery-evidence-packet-ndb.md");
    expect(record.rtoRpoVarianceReviewReference).toBe("production-archive-rto-rpo-variance-review-ndb.md");
    expect(record.residualRecoveryRiskRegisterReference).toBe("production-archive-residual-recovery-risk-register-ndb.md");
    expect(record.acceptanceSignOffReference).toBe("production-archive-recovery-acceptance-signoff-ndb.md");
    expect(record.provisioningEnabled).toBe(false);
    expect(record.killSwitch.enabled).toBe(false);
  });
});

function sampleArchiveRecoveryDrillRecord(
  status: "Blocked" | "Ready for production execution archive recovery drill review"
): ProductionExecutionArchiveRecoveryDrillRecord {
  return {
    id: "production-execution-archive-recovery-drill-record-ndb-1",
    provider: "NDB",
    archiveRetrievalValidationRecordId: "production-execution-archive-retrieval-validation-record-ndb-1",
    ...
    drillOwner: "Production Archive Recovery Drill Owner",
    recoveryScenarioReference: "production-archive-recovery-scenario-ndb.md",
    elapsedRecoveryProofReference: "production-archive-elapsed-recovery-proof-ndb.md",
    restoredArtifactReviewReference: "production-archive-restored-artifact-review-ndb.md",
    drillSignOffReference: "production-archive-recovery-drill-signoff-ndb.md",
    checks: [],
    evidence: [],
