...
import type { ProductionExecutionArchiveRecoveryClosureRecord } from "../src/data/cloudStudioDomain";
import {
...
describe("production execution archive recovery audit certification record", () => {
  it("requires a production execution archive recovery closure record", () => {
    const state = createDefaultState();

    expect(() =>
      createProductionExecutionArchiveRecoveryAuditCertificationRecord(state, {}, "platform.admin")
    ).toThrow(ProductionExecutionArchiveRecoveryAuditCertificationRecordError);
  });

  it("blocks archive recovery audit certification when closure or certification evidence is incomplete", () => {
    const state = createDefaultState();
    state.productionExecutionArchiveRecoveryClosureRecords = [sampleArchiveRecoveryClosureRecord("Blocked")];

    const record = createProductionExecutionArchiveRecoveryAuditCertificationRecord(
      state,
      {
  ...
  it("marks archive recovery audit certification ready when closure and certification evidence are complete", () => {
    const state = createDefaultState();
    state.productionExecutionArchiveRecoveryClosureRecords = [
      sampleArchiveRecoveryClosureRecord("Ready for production execution archive recovery closure review"),
    ];
    ...
    expect(record.status).toBe("Ready for production execution archive recovery audit certification review");
    expect(record.certificationOwner).toBe("Production Archive Recovery Audit Certification Owner");
    expect(record.auditEvidenceManifestReference).toBe(
      "production-archive-recovery-audit-evidence-manifest-ndb.json"
    );
    expect(record.controlMappingReviewReference).toBe("production-archive-recovery-control-mapping-review-ndb.md");
    expect(record.exceptionDispositionReference).toBe("production-archive-recovery-exception-disposition-ndb.md");
    expect(record.auditCertificationSignOffReference).toBe(
      "production-archive-recovery-audit-certification-signoff-ndb.md"
    );
...
function sampleArchiveRecoveryClosureRecord(
  ...
  return {
    id: "production-execution-archive-recovery-closure-record-ndb-1",
    provider: "NDB",
    archiveRecoveryAcceptanceRecordId: "production-execution-archive-recovery-acceptance-record-ndb-1",
    ...
    closureOwner: "Production Archive Recovery Closure Owner",
    recoveryClosurePacketReference: "production-archive-recovery-closure-packet-ndb.md",
    followUpActionRegisterReference: "production-archive-recovery-follow-up-actions-ndb.md",
    stakeholderClosureNoticeReference: "production-archive-recovery-stakeholder-closure-notice-ndb.md",
    archiveRecoveryClosureSignOffReference: "production-archive-recovery-closure-signoff-ndb.md",
    checks: [],
    evidence: [],
