...
- Real-infrastructure tester enablement adds private `.env.lab` generation, provider-specific readiness validation, safer Docker host ports, and tester documentation for Prism Element or Prism Central labs.
- Browser-based infrastructure connection setup adds a one-time Prism Element or Prism Central read-only connection test in Admin Settings, with password redaction and lifecycle mutation disabled.
...
- `v2.63.0-production-execution-archive-recovery-closure-record` adds evidence-only archive recovery closure records linked to archive recovery acceptance records and an Admin Operations archive recovery closure panel.
- `v2.62.0-production-execution-archive-recovery-acceptance-record` adds evidence-only archive recovery acceptance records linked to archive recovery drill records and an Admin Operations archive recovery acceptance panel.
- `v2.61.0-production-execution-archive-recovery-drill-record` adds evidence-only archive recovery drill records linked to archive retrieval validation records and an Admin Operations archive recovery drill panel.
...
- `v2.52.0-production-execution-support-readiness-record` adds evidence-only support readiness records linked to operations handovers and an Admin Operations support readiness panel.
- `v2.51.0-production-execution-operations-handover-record` adds evidence-only operations handover records linked to completion dossiers and an Admin Operations operations handover panel.
